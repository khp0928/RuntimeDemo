...
//                                     @"title": [DoraemonUtil dateFormatNow].length > 0 ? [DoraemonUtil dateFormatNow] : @"",
    ...
    while (!self.cancelled) {
        if (_isApplicationInActive) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{ /// 如果发生了卡顿。。主线程不会得到执行。。。
...
@interface Monitoring_ping ()

@property(nonatomic,strong) PingThread * pingThread;/**ins*/

@end

@implementation Monitoring_ping

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
//    self.pingThread = [[PingThread alloc] initWithThreshold:threshold
